...
function UserMenu(){
    ...
    return(
        <div className='d-flex gap-05 relative ml-auto'>
            <span className="h-2rem w-2rem border-grey-01 rounded-100px bg-common-blue d-grid center-y justify-center hide-overflow">
                {(user && user.image) && <img className="w-2rem" src={`${import.meta.env.VITE_REACT_APP_API_URL}\\${user.image}`} alt="profile-picture" />}
            </span>
